...
import { Course, Purchase, Chapter, UserProgress, Payment } from "@prisma/client";
    ...
    const courseAnalytics = courses.map((course) => {
      try {
        ...
        const courseRevenue = course.purchases.reduce((total: number, purchase) => {
          if (purchase.payment && purchase.payment.status === "COMPLETED") {
            return total + (purchase.payment.amount || 0);
          }
          return total;
        }, 0);
